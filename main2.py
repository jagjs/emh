...
api_key =  'this is where the api key goes'
...
#rounding to nearest 15 minute, this matches the interval that we get stock data for and ensures each row of data has the correct corresponding sentiment score
    ...
    for run in range(3):
        ...
        early_stopping = EarlyStopping(monitor='val_mae', mode='min', patience=20, restore_best_weights=True)
        ...
        # Define global storage for use in directional accuracy
        ...
        #MUST MAKE AMENDS TO THE GLOBAL PART OF THE CODE
        ...
        def objective(trial, model_fn, model_name):
            tf.keras.backend.clear_session()
            ...
            for repeat in range(num_repeats):
                ...
                # Train the model
                history = model.fit(dataset,epochs=100, validation_data=val_dataset,  callbacks=[early_stopping, checkpoint])
        ...
        for model_name, model_fn in model_builders.items():
            ...
            study.optimize(lambda trial: objective(trial, model_fn, model_name), n_trials=1)
        ...
        if best_run['model_name'] == 'Model_1_TrueRandomWalk':
            ...
            early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_mae', patience=20, mode='min', restore_best_weights=True)
